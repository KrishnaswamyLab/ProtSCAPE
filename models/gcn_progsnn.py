...
class ProGSNN_GCN(TGTransformerBaseModel_ATLAS):

    def __init__(self, hparams):
        super(ProGSNN_GCN, self).__init__(hparams)
        ...
        self.gcn1 = GCNConv(in_channels=self.input_dim, out_channels=self.hidden_dim)
        self.gcn2 = GCNConv(in_channels=self.hidden_dim, out_channels=self.hidden_dim)
    ...
    def encode(self, batch):
        ...
        gcn_output = self.gcn2(x, edge_index)
        # import pdb; pdb.set_trace()
        print("Scattering completed!")
        #Row transformer encoding outputs attention map of shape [100,1,204,204]
        row_output_embed, att_maps = self.row_transformer_encoding(gcn_output)
        ...
        # import pdb; pdb.set_trace() 
        col_output_embed, attention_maps = self.col_transformer_encoding(gcn_output)
