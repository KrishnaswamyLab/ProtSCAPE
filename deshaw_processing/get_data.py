...
for entry in tqdm(range_to_filename.keys()):
    ...
    pdbs = np.load(f"/gpfs/gibbs/pi/krishnaswamy_smita/sv496/ProGSNN-2/file_names_UB/{range_to_filename[entry]}.npy")
    ...
    constructors = {
        ...
        "pdb_dir": f"/gpfs/gibbs/pi/krishnaswamy_smita/de_shaw/Ubiquitin/{entry}",
    ...
    for idx, pdb in enumerate(tqdm(pdbs)):
        path = f"/gpfs/gibbs/pi/krishnaswamy_smita/de_shaw/Ubiquitin/{entry}" + '/' + pdb
    ...
    #         print(i)
    #         pyg_list.remove(i)
    with open(f"/gpfs/gibbs/pi/krishnaswamy_smita/sv496/ProGSNN-2/deshaw_processing/graphs_ub/{range_to_graphname[entry]}.pkl", "wb") as file:
        pickle.dump(pyg_list, file)

print("combining data")
total_graphs = []
arr = os.listdir("/gpfs/gibbs/pi/krishnaswamy_smita/sv496/ProGSNN-2/deshaw_processing/graphs_ub")
for i, entry in enumerate(tqdm(arr)):

    split1, split2 = entry.split('.')
    if split2 == 'pkl' and split1 != 'total_graphs':

        with open("/gpfs/gibbs/pi/krishnaswamy_smita/sv496/ProGSNN-2/deshaw_processing/graphs_ub/" + entry, "rb") as file:
            graphs = pickle.load(file)
            for graph in graphs:
                total_graphs.append(graph)

with open("/gpfs/gibbs/pi/krishnaswamy_smita/sv496/ProGSNN-2/deshaw_processing/graphs_ub/total_graphs.pkl", 'wb') as out:
